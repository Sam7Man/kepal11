...
use App\Order;
use App\Detailorder;
use App\Http\Helpers\DSA_Transaction;
use App\Rekening;
...
class OrderController extends Controller
{
    ...
    public function simpan(Request $request)
    {
        ...
        if($cek_invoice < 1){
            ...
            $dsa = new DSA_Transaction($request->pesan);
            $result = $dsa->makeDSA();
        if($request->metode_pembayaran == 'cod'){
            Order::create([
                ...
                'biaya_cod' => 10000,
                'no_hp' => $request->no_hp,
                'pesan' => $result['msg_crpt'],
                'private' => $result['prvt_ky'],
                'encrypt_key' => $result['encrypt_ky'],
                'signaiv' => $result['sgivnat']
            ]);
        }else{
